defmodule Charon.TokenFactory.Jwt do
  ...
  def sign(payload, config) do
    ...
    with {:ok, _key = {alg, secret}} <- config |> get_keyset.() |> get_key(kid),
      ...
      nonce = maybe_generate_nonce(alg)
      key = {alg, maybe_gen_otk(secret, nonce)}
  ...
  defp maybe_generate_nonce(:poly1305), do: :crypto.strong_rand_bytes(12)
  defp maybe_generate_nonce(_), do: nil
  ...
  defp maybe_add_nonce(map, nil), do: map
  defp maybe_add_nonce(map, nonce), do: Map.put(map, :nonce, url_encode(nonce))
  ...
  defp maybe_gen_otk(secret, nil), do: secret

  defp maybe_gen_otk(secret, nonce) do
